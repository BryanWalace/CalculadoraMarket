...
- `expo-haptics` — feedback tátil ao adicionar item e ao ultrapassar o orçamento
...
- Instalar `expo-sqlite` ou `expo-asset` direto via `npm`/`expo install` falha com ERESOLVE por causa de pacotes web opcionais do próprio `expo-router` (`@expo/ui`, `@radix-ui/*`, `vaul`) exigindo uma versão de `react-dom` diferente da resolvida — irrelevante para o app mobile. O `.npmrc` do projeto já fixa `legacy-peer-deps=true` (necessário também para `npm ci` no CI, que por padrão é mais estrito que `npm install`), então basta instalar normal (`npx expo install <pacote>`), sem precisar repetir a flag na mão.
...
- **Nunca importar de `@react-navigation/*` direto** — desde o SDK 56 o expo-router não é mais compatível com isso (falha só na hora de empacotar um build de release/produção, `createBundleReleaseJsAndAssets`; no `expo start`/dev client não dá erro nenhum, então passa despercebido até o primeiro build de verdade). `ThemeProvider`, `DarkTheme` e `DefaultTheme` (usados em `app/_layout.tsx`) vêm direto de `expo-router` agora, mesma API. Não há mais `@react-navigation/native` no `package.json`.
- Arquivos de teste que moram dentro de `app/` (convenção deste projeto, ex. `app/index.test.tsx`) são tratados pelo expo-router como rota em potencial — o Jest não liga (resolve os testes direto), mas o bundle de produção tenta empacotar `@testing-library/react-native` junto e quebra (`Unable to resolve module "console"`). `metro.config.js` bloqueia `*.test.{js,jsx,ts,tsx}` do resolver do Metro para isso não acontecer. Validar mudança de rota/import com `npx expo export -p android --output-dir <pasta>` antes de confiar num build nativo de release — é bem mais rápido que esperar o Gradle e pega os dois problemas acima cedo.
