...
- [ ] **T-11** — **TDD** `src/lib/money.ts`: testes de `multiplyCents` (incluindo arredondamento) antes da implementação. _(RF-07, plan.md ADR-02)_
...
- [ ] **T-20** — Editar item existente: toque no item reabre a tela de confirmação pré-preenchida em modo edição. _(RF-10)_
...
- [ ] **T-26** — Instalar/configurar `expo-camera`; tela explicativa de permissão antes da primeira solicitação (só pedida no momento do uso). _(RF-12, RF-54)_
...
- [ ] **T-33** — **TDD** `src/lib/ocr-parser.ts`: testes de extração de nome (linha alfabética mais longa, exclusão de palavras de ruído) e Title Case. _(RF-20, RF-21)_
...
- [ ] **T-42** — Diálogo de finalizar compra: campo opcional "Loja" e nome sugerido (loja + data), editável. _(RF-38)_
